...
const Category: React.FC<CategoryProps> = ({ label, icon: Icon, selected }) => {
  ...
  const handleClick = useCallback(() => {
    if (label == 'All') {
      // click to category "All" push user back to home page
      router.push('/');
    } else {
      let currentQuery = {};
      ...
      // update object from params to the new one
    ...
    // pass the dependecies, when label, params and router change, we call this function again
  }, [label, params, router]);
